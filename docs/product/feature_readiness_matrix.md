...
| File attachment and vision screenshot | Chat composer attach and screenshot buttons | BETA | Upload/result UI is covered; image uploads now fail honestly with `vision_provider_required` until a Vision provider is configured; screenshot analysis preflights readiness before the critical bridge path. Public privacy review is not final. | Users may assume full image analysis works before provider/API selection exists. | Keep visible with internal/beta posture; do not market as full Vision until provider choice, privacy posture, and real API tests are complete. | Optional | Yes, after privacy review and provider proof |
...
| Agent runs and attention filter | Chat composer `/agent`, hidden attention filter, agent completion panels | BETA | Backend safety gates are adversarially covered; the chat surface now uses softer sender/badge/step labels while preserving technical detail in hidden metadata. Runtime autonomy remains gated and attention UI appears when runs need review. | Users may expect autonomous completion without review, or lose trust if raw tool names leak into normal chat. | Keep as beta/internal workflow; continue polishing UX, but do not expose as public automation promise. | Optional for operators | Yes, after agent policy approval |
