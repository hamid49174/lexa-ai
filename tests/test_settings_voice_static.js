...
const changeSttModelSource = extractFn(src, "changeSttModel");
const changeSttEngineSource = extractFn(src, "changeSttEngine");
...
const secretActionSource = extractFn(src, "runSettingsSecretAction");
const secretBusySource = extractFn(src, "setSettingsSecretActionsBusy");
...
const setDeepgramKeySource = extractFn(src, "setDeepgramKeyAction");
const deleteDeepgramKeySource = extractFn(src, "deleteDeepgramKeyAction");
const setCartesiaKeySource = extractFn(src, "setCartesiaKeyAction");
const deleteCartesiaKeySource = extractFn(src, "deleteCartesiaKeyAction");
const elevenlabsKeySource = extractFn(src, "elevenlabsKeyAction");
...
assert("settings refresh ignores stale async responses", src.includes("let _settingsRefreshSeq = 0") && src.includes("const requestId = ++_settingsRefreshSeq") && src.includes("await Promise.allSettled([") && src.includes("if (requestId !== _settingsRefreshSeq || !LexaState.get(\"backendOnline\")) return") && src.indexOf("if (requestId !== _settingsRefreshSeq || !LexaState.get(\"backendOnline\")) return") > src.indexOf("await Promise.allSettled([") && src.indexOf("if (requestId !== _settingsRefreshSeq || !LexaState.get(\"backendOnline\")) return") < src.indexOf("const ai = aiRes.status"));
...
assert("settings STT selectors block duplicate backend writes and restore select state", src.includes("let _sttModelChangeRunning = false") && src.includes("let _sttEngineChangeRunning = false") && changeSttModelSource.includes("if (!modelName || _sttModelChangeRunning) return") && changeSttModelSource.includes('const select = document.getElementById("stt-model-select")') && changeSttModelSource.includes("setSettingsActionBusy(select, true)") && changeSttModelSource.includes("await window.lexa.sttSetModel(modelName)") && changeSttModelSource.includes("_sttModelChangeRunning = false") && changeSttModelSource.includes("setSettingsActionBusy(select, false)") && changeSttEngineSource.includes("if (!engine || _sttEngineChangeRunning) return") && changeSttEngineSource.includes('const select = document.getElementById("stt-engine-select")') && changeSttEngineSource.includes("setSettingsActionBusy(select, true)") && changeSttEngineSource.includes("await window.lexa.sttSetEngine(engine)") && changeSttEngineSource.includes("_sttEngineChangeRunning = false") && changeSttEngineSource.includes("setSettingsActionBusy(select, false)"));
assert("settings secret key actions are singleflight guarded through modal and keyring writes", src.includes("let _settingsSecretActionRunning = false") && src.includes("const SETTINGS_SECRET_ACTIONS = [") && src.includes('"setDeepgramKey"') && src.includes('"deleteDeepgramKey"') && src.includes('"setCartesiaKey"') && src.includes('"deleteCartesiaKey"') && src.includes('"elevenlabsKeyAction"') && secretBusySource.includes("SETTINGS_SECRET_ACTIONS.forEach") && secretBusySource.includes("setSettingsActionButtonsBusy(actionName, busy)") && secretActionSource.includes("if (_settingsSecretActionRunning) return") && secretActionSource.includes("_settingsSecretActionRunning = true") && secretActionSource.includes("setSettingsSecretActionsBusy(true)") && secretActionSource.includes("await actionFn()") && secretActionSource.includes("finally") && secretActionSource.includes("_settingsSecretActionRunning = false") && secretActionSource.includes("setSettingsSecretActionsBusy(false)") && setDeepgramKeySource.includes('await runSettingsSecretAction(async () =>') && deleteDeepgramKeySource.includes('await runSettingsSecretAction(async () =>') && setCartesiaKeySource.includes('await runSettingsSecretAction(async () =>') && deleteCartesiaKeySource.includes('await runSettingsSecretAction(async () =>') && elevenlabsKeySource.includes('await runSettingsSecretAction(async () =>'));
