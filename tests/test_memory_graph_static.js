...
assert("Renderer ignores stale graph refresh results", memorySrc.includes("let memoryGraphRefreshSeq = 0") && memorySrc.includes("const requestId = ++memoryGraphRefreshSeq") && memorySrc.includes('LexaState.get("currentView") !== "memory"') && memorySrc.includes("if (requestId !== memoryGraphRefreshSeq || LexaState.get(\"currentView\") !== \"memory\") return") && memorySrc.indexOf("if (requestId !== memoryGraphRefreshSeq || LexaState.get(\"currentView\") !== \"memory\") return") > memorySrc.indexOf("await window.lexa.memoryGraph(180)") && memorySrc.indexOf("if (requestId !== memoryGraphRefreshSeq || LexaState.get(\"currentView\") !== \"memory\") return") < memorySrc.indexOf("renderMemoryGraph(graphData)"));
...
assert("Renderer adds a dense neural-cloud backdrop without turning decorative nodes into graph nodes", memorySrc.includes("function memoryGraphCreateNeuralCloud(") && memorySrc.includes("memory-graph-neural-dot") && memorySrc.includes("memory-graph-neural-filament"));
...
assert("Graph CSS styles the blue neural cloud surface", [".memory-graph-neural-cloud", ".memory-graph-neural-dot", ".memory-graph-neural-filament"].every((selector) => viewsCss.includes(selector)));
