...
async function main() {
  ...
  const result = await runRenderer(win, `
    (async () => {
      ...
      const neuralDots = document.querySelectorAll("#memory-graph-svg .memory-graph-neural-dot").length;
      const neuralFilaments = document.querySelectorAll("#memory-graph-svg .memory-graph-neural-filament").length;
  ...
  assert("Memory graph renders dense neural cloud backdrop", Number(result.neuralDots || 0) >= 120 && Number(result.neuralFilaments || 0) >= 160, JSON.stringify(result));
