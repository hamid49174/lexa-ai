...
assert("manual Personal OS refresh can supersede stale in-flight queues", personalOsSrc.includes("let personalOsRefreshSeq = 0") && personalOsSrc.includes("const requestId = ++personalOsRefreshSeq") && personalOsSrc.includes("if (auto && PersonalOSState.isRefreshing) return false") && !personalOsSrc.includes("if (PersonalOSState.isRefreshing) return false;") && personalOsSrc.includes("if (requestId !== personalOsRefreshSeq) return false") && personalOsSrc.includes("if (requestId === personalOsRefreshSeq) PersonalOSState.isRefreshing = false") && personalOsSrc.indexOf("if (requestId !== personalOsRefreshSeq) return false") > personalOsSrc.indexOf("await Promise.all([") && personalOsSrc.indexOf("if (requestId !== personalOsRefreshSeq) return false") < personalOsSrc.indexOf("renderPersonalOsStatus(status"));
