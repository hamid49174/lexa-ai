...
assert("bridge security errors use product-safe UI copy", preloadSrc.includes("function bridgeSecurityDisplayMessage") && preloadSrc.includes("Diese lokale Aktion wurde nicht gestartet") && preloadSrc.includes("This local action was not started") && !preloadSrc.includes("High-risk action requires explicit user presence") && !preloadSrc.includes("User presence challenge could not be consumed") && !preloadSrc.includes("executeBatch only allows explicitly read-only companion commands"));
