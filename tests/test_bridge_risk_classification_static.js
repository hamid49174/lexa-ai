...
assert("clipboard history reveal stays explicit", memorySrc.includes('button.addEventListener("click", (event) => revealClipboardHistory(event.currentTarget))') && memorySrc.includes("window.lexa.clipboardHistory()"));
assert("clipboard history reveal is singleflight guarded", memorySrc.includes("let _clipboardHistoryRevealRunning = false") && memorySrc.includes("if (_clipboardHistoryRevealRunning) return") && memorySrc.includes("_clipboardHistoryRevealRunning = true") && memorySrc.includes('setMemoryActionBusy(triggerBtn, true)') && memorySrc.includes('console.warn("[Memory] Failed to reveal clipboard history:"') && memorySrc.includes("_clipboardHistoryRevealRunning = false") && memorySrc.includes("if (triggerBtn?.isConnected) setMemoryActionBusy(triggerBtn, false)"));
