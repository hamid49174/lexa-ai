...
def test_health_accepts_http_only_electron_cookie_without_leaking_token(monkeypatch):
    monkeypatch.setenv("LEXA_INSTANCE_TOKEN", "unit-secret")
    ...
    client = TestClient(main.app)
    client.cookies.set("lexa_local_auth", "unit-secret")
    res = client.get("/health")
...
def test_privileged_endpoint_accepts_valid_electron_cookie(monkeypatch):
    monkeypatch.setenv("LEXA_INSTANCE_TOKEN", "unit-secret")
