...
assert("chat composer palette file is classic helper script", chatComposerPaletteSrc.includes("function setupComposerCommandPalette(") && chatComposerPaletteSrc.includes("function handleComposerCommandKeydown(") && !chatComposerPaletteSrc.includes("function renderConversationStarters(") && !/(^|\n)\s*(import|export)\b/.test(chatComposerPaletteSrc));
assert("chat message actions file is classic helper script", chatMessageActionsSrc.includes("function workspaceDraftPromptFromText(") && chatMessageActionsSrc.includes("function continuePromptFromText(") && !/\b(import|export)\b/.test(chatMessageActionsSrc));
...
assert("extracted formatting helpers remain consumed", !chatSrc.includes("function stripModelFunctionTags(") && !chatSrc.includes("function normalizeChatUrl(") && chatMessageFormattingSrc.includes("stripModelFunctionTags(text)") && chatMarkdownSrc.includes("normalizeChatUrl(url)"));
