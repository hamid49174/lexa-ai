...
- [x] **C. Chat & KI-Kern** (Kern erledigt + verifiziert, 2026-06-26) — Tiefen-Audit (14 safe/18 med/3 risky). **C-Verifikation: 0 Regressionen.** Batch 1+2 gefixt (Commits a23416c5, 490e56ac): Vision-Decompression-Bomb-Schutz, URL-Klammern, lexa-sources-Strip, toter Code (process_tool_calls/renderStreamingText), **48 exakte Quality-Mode-Marker-Duplikate entfernt** (138 Quality-Tests grün), /chat/file-Verlaufs-Hygiene. **BEWUSST GEFLAGGT statt eigenmächtig (Architektur-Entscheidung des Users): Entfernung des toten Groq/Anthropic-CHAT-Provider-Codes (~400 Z.) — laut Docstring absichtliche „Reserve", null Funktionsgewinn (unerreichbar), aber Risiko für den aktiven Gemini-Dispatch + Anthropic ist test-gekoppelt. = „Gemini-only endgültig festschreiben & löschen" vs. „Reserve behalten".** OFFEN/niedrig: Quality-Mode datengetrieben (medium, kosmetisch), pending-confirmation set-Bindung (medium, get-Seite schon gebunden).
- [ ] D. Voice & Sprache
- [ ] E. Multi-Agenten-Orchestrator
- [ ] F. Gedächtnis & Wissen
- [ ] G. Produktivität
