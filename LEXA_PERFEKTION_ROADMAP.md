...
- [~] **B. PC-Steuerung & Desktop-Automation** (Kern erledigt, 2026-06-26) — Tiefen-Audit (13 high/24 med/11 low). Gefixt (Commits 6de66d82 + 92767dc6, Suite 1818 grün): Fenster-Fokus robust gegen Foreground-Lock (AttachThreadInput+Retry), click fokussiert Zielfenster + Guard, ehrliche click-Verifikation, generischer App-Launcher, shutdown_cancel erreichbar; archive_extract Datenverlust-Schutz (overwrite-Gate), file_open-Blocklist gehärtet (.lnk/.reg/.scr/.msi/.com/.dll/.sys), improve_lexa_with_hermes propose-only, docker/git returncode-Checks. **OFFEN (Rest-Audit, meist live-Windows-verifizierungsbedürftig): window_layout snappt Vordergrund (PowerShell) statt Zielfenster (#7), OCR-Capture ohne Fokus (#9), spotify_open Pixel-Heuristik→UIA (#8), batch_rename Registry-Signatur (#4), Fenster-Disambiguierung bei Mehrfachtreffer, service_stop -Force-Warnung, open_url Page-Leak, + Mediums/Lows. env_list-Tier = needs_user.** batch_rename prefix/suffix jetzt erreichbar (Registry, Commit folgt). **Adversarische B-Verifikation: 0 Regressionen** (Fokus/Click, Dateien, Registry/Hermes sauber). Rest-Punkte = live-Windows-verifizierungsbedürftig/niedrig → fuer echte Windows-Sitzung vorgemerkt.
- [~] **C. Chat & KI-Kern** (läuft, 2026-06-26) — Tiefen-Audit (14 safe/18 med/3 risky). **WICHTIG: „800 Z. toter Provider-Code" aus dem ersten Scan stimmt so NICHT** — _get_groq_client wird von vision.py genutzt, _get_openai_client/_chat_openai_compatible vom Embeddings-/Gemini-Pfad; nur die Groq/Anthropic-CHAT-Wrapper + tote Provider-Zweige sind echt entfernbar (Anthropic test-gekoppelt=risky, Groq=medium, entflochten anzugehen). Batch 1 erledigt (Commit a23416c5, 1820/35 grün): Vision-Decompression-Bomb-Schutz (Groq-Bytes-Pfad), nackte-URL-Klammern, lexa-sources-Strip nur assistant/system, toter Code (process_tool_calls/renderStreamingText). **OFFEN: Marker-Dedup (77 exakte Duplikate, vom Audit als test-grün verifiziert — größter Entrümpel-Win), /chat/file speichert full_prompt als User-History, Groq/Anthropic-Chat-Wrapper + tote Provider-Zweige entflochten entfernen, Quality-Mode datengetrieben (medium), Pending-Confirmation set-Bindung (medium). Dann adversarische C-Verifikation.**
- [ ] D. Voice & Sprache
- [ ] E. Multi-Agenten-Orchestrator
- [ ] F. Gedächtnis & Wissen
- [ ] G. Produktivität
